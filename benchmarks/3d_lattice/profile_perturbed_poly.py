...
print(f"Setting up 3D perturbed lattice POLYDISPERSE profile with {N} particles (2 wavenumbers)...")
...
warmup_eps_p_mixed = np.where(warmup_choices[None, :] == 0, eps_p1[:, None], warmup_eps_p_mixed.shape[1] if 'warmup_eps_p_mixed' in locals() else eps_p2[:, None])
# Wait, let's just use:
